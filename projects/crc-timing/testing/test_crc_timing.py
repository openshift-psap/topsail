...
def start_macos_vm(base_work_dir):
    ...
    prepare_crc_timing.kill_vfkit(base_work_dir)
...
def wait_vm_crc_ready(base_work_dir, ip_addr):
    ...
    while True:
        ...
        if "SUCCESS" in out:
            ...
            raise RuntimeException("'crc-custom.target' failed to get ready")
        else:
            logging.info(f"wait_vm_crc_ready: command not running after {elapsed}s")
            if elapsed > DELAY:
                raise RuntimeException("failed to check 'crc-custom.target' status ")
...
def execute_vm_command(base_work_dir, ip_addr, command, ssh_timeout=None, handled_secretly=False, capture_stdout=False, check=False):
