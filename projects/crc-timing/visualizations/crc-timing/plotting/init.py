...
def generateInitServicesData(entries, variables, ordered_vars):
    ...
    for entry in entries:
        ...
        for name, unit in entry.results.systemd_units.items():
            ...
            if name.removesuffix(".service") in entry.results.systemd_journal_duration:
...
class SystemdInitTiming():
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        for index, row in df.iterrows():
            if not (row['Name'].startswith("ocp-") or row['Name'].startswith("crc-")): continue
