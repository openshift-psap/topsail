...
class PodmanMachine:
    ...
    def start(self):
        ...
        if is_windows:
            ...
            cmd = f"""
$scriptContent = @"
& {get_podman_binary(self.base_work_dir)} machine start {self.machine_name} *>&1 | Out-File -FilePath "`$env:USERPROFILE\\podman_script_log.txt"
"@

Set-Content -Path "$env:USERPROFILE\\start_podman.ps1" -Value $scriptContent
$command = "powershell.exe -ExecutionPolicy Bypass -File `"$env:USERPROFILE\\start_podman.ps1`""

$result = Invoke-WmiMethod -Class Win32_Process -Name Create -ArgumentList $command

# Wait for podman machine to be fully ready using while loop
Write-Output "Waiting for podman machine to boot..."
...
while ($elapsed -lt $timeout) {{
    try {{
        & {get_podman_binary(self.base_work_dir)} info *>$null
        if ($LASTEXITCODE -eq 0) {{
            Write-Output "Podman machine is ready after $elapsed seconds"
            break
        }}
    }} catch {{
        # Continue waiting
...
if ($elapsed -ge $timeout) {{
    Write-Output "Warning: Timeout waiting for podman machine to be ready"
    type $env:USERPROFILE\podman_script_log.txt
}}

Remove-Item "$env:USERPROFILE\start_podman.ps1" -Force -ErrorAction SilentlyContinue
"""  # noqa: E501
            # -------------
        remote_access.run_with_ansible_ssh_conf(self.base_work_dir, cmd)
...
class DockerDesktopMachine:
    ...
    def start(self):
        ...
        if is_windows:
            ...
            cmd = f"""
$scriptContent = @"
& {cmd} *>&1 | Out-File -FilePath "`$env:USERPROFILE\\docker_script_log.txt"
"@

Set-Content -Path "$env:USERPROFILE\\start_docker.ps1" -Value $scriptContent
$command = "powershell.exe -ExecutionPolicy Bypass -File `"$env:USERPROFILE\\start_docker.ps1`""
...
while ($elapsed -lt $timeout) {{
    try {{
        & docker info *>$null
        if ($LASTEXITCODE -eq 0) {{
            Write-Output "Docker machine is ready after $elapsed seconds"
            break
        }}
    }} catch {{
        # Continue waiting
...
if ($elapsed -ge $timeout) {{
    Write-Output "Warning: Timeout waiting for docker desktop to be ready"
    type $env:USERPROFILE\docker_script_log.txt
}}

Remove-Item "$env:USERPROFILE\start_docker.ps1" -Force -ErrorAction SilentlyContinue
"""  # noqa: E501
            # -------------
        remote_access.run_with_ansible_ssh_conf(self.base_work_dir, cmd)
