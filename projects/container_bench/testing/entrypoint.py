...
from constants import CRC_CONTAINER_BENCH_SECRET_PATH, TESTING_THIS_DIR
...
def init(ignore_secret_path=False, apply_preset_from_pr_args=True):
    ...
    if not ignore_secret_path:
        if not CRC_CONTAINER_BENCH_SECRET_PATH.exists():
            raise RuntimeError(
                f"Path with the secrets (CRC_CONTAINER_BENCH_SECRET_PATH={CRC_CONTAINER_BENCH_SECRET_PATH}) "
                "does not exists."
            )
        run.run(f'sha256sum "$CRC_CONTAINER_BENCH_SECRET_PATH"/* > "{env.ARTIFACT_DIR}/secrets.sha256sum"')
