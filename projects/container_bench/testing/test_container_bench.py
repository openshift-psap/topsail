...
def test_all_benchmark():
    ...
    for platform_str in all_platforms_str:
        ...
        logging.info(f"Benchmakrs: {benchmarks}")
        logging.info(f"Run platform: {platform_str}")
        with env.NextArtifactDir(f"{platform_str}_benchmarks".replace("/", "_")):
...
def prepare_benchmark_args(platform, benchmark, base_work_dir):
    ...
    benchmark_kwargs = dict(
        working_dir=base_work_dir / benchmark.name,
        runtime=c.engine,
