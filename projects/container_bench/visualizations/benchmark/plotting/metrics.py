...
def register():
    MetricUsage(CPUUsage=True, NetworkUsage=False, DiskUsage=False, PowerUsage=False)
    MetricUsage(CPUUsage=False, NetworkUsage=True, DiskUsage=False, PowerUsage=False)
    MetricUsage(CPUUsage=False, NetworkUsage=False, DiskUsage=True, PowerUsage=False)
    MetricUsage(CPUUsage=False, NetworkUsage=False, DiskUsage=False, PowerUsage=True)
    ...
    for entry in entries:
        ...
        if secondary_key == "network":
            ...
            for usage, t in zip(cpu_usage, cpu_time_points):
                ...
                data.append(entry_data)
        elif secondary_key == "power":
            power_usage = main_field.power
            power_time_points = [i * interval for i in range(len(power_usage))]
            for usage, t in zip(power_usage, power_time_points):
                entry_data = {
                    "name": entry_name,
                    "ts": t,
                    "usage": usage
...
class MetricUsage():
    def __init__(self, CPUUsage=True, NetworkUsage=False, DiskUsage=False, PowerUsage=False):
        self.name = "System"
        if NetworkUsage:
