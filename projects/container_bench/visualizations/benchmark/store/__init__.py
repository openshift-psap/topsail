...
class KserverBenchamrkStore(helpers_store.BaseStore):
    def prepare_for_pickle(self, results):
        if results.benchmark_config:
            results.benchmark_config.get = None

    def prepare_after_pickle(self, results):
        if results.benchmark_config:
            results.benchmark_config.get = helpers_store.get_yaml_get_key(
                results.benchmark_config.name,
                results.benchmark_config.yaml_file
            )
...
def _rewrite_settings(settings_dict, results=None, is_lts=None):
    ...
    test_benchmark = settings_dict.pop("test.platform", results.test_config.get("test.benchmark"))
