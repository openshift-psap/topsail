...
class PodNodeMappingReport():
    ...
    def do_plot(self, *args):
        ...
        for what in "Notebooks", "Test Pods":
            ...
            header += [report.Plot(f"Pod/Node timeline: {what}", args)]
            ...
            header += [report.Plot(f"Pod/Node timeline: {what}", report.set_config(dict(force_order_by_user_idx=True),
                                                                         args))]
            ...
            header += [report.Plot(f"Pod/Node distribution: {what}", args)]
            ...
            header += [report.Plot(f"Pod/Node performance index: {what}", args)]
        ...
        header += [report.Plot(f"Test nodes test duration", args)]
...
class UserExecutionOverviewReport():
    ...
    def do_plot(self, *args):
        ...
        header += [report.Plot("Notebook spawn time", args)]
        ...
        header += [report.Plot(f"Notebook spawn time", report.set_config({"hide_launch_delay": True}, args))]
        ...
        header += [report.Plot(f"Notebook spawn time", report.set_config({"keep_failed_steps": True}, args))]
        ...
        header += [report.Plot(f"Notebook spawn time", report.set_config({"hide_failed_users": True}, args))]
        ...
        header += [report.Plot(f"Notebook Resource Creation Timeline", args)]
        ...
        header += [report.Plot(f"Notebook Resource Creation Delay", args)]
    ...
    def do_plot(self, *args):
        ...
        def add_price(user_count):
            ...
            for mode in ["notebooks", "test_pods"]:
                header += [report.Plot("Price to completion",
                                report.set_config(dict(mode=mode, user_count=user_count), args))]
...
class LaunchAndExecTimeDistributionReport():
    ...
    def do_plot(self, *args):
        ...
        header += report.Plot_and_Text("Launch time distribution", args)
        ...
        for entry in common.Matrix.all_records(settings, setting_lists):
            ...
            if entry.results.user_count == 1:
                header += report.Plot_and_Text("Notebook Performance",
                                        report.set_config(dict(user_details=1, stacked=1), report.set_entry(entry, args)))
                ...
                header += report.Plot_and_Text("Notebook Performance", report.set_config(dict(all_in_one=True), report.set_entry(entry, args)))
                header += ["This plot shows the distribution of the notebook performance benchmark results, "
                           "for all of the simulated users."]
                header += html.Br()
                header += html.Br()

                header += report.Plot_and_Text("Notebook Performance", args)
                header += ["This plot shows the distribution of the notebook performance benchmark results,"
