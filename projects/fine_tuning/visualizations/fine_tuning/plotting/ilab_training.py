...
def generateILabSummaryData(entries, x_key, _variables, summary_key, compute_speedup=False, filter_key=None, filter_value=None, y_lower_better=True):
    data = []
...
class ILabSummary():
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        data, has_speedup = generateILabSummaryData(entries, x_key, variables, cfg__summary_key, compute_speedup, cfg__filter_key, cfg__filter_value, y_lower_better)
        ...
        if has_gpu or has_speedup:
            ...
            do_line_plot = False
        elif x_key.startswith("hyper_parameters.") or x_key == "pod_count":
            do_line_plot = True
        else:
            do_line_plot = False
...
def generateILabProgressData(entries, x_key, variables, progress_key):
    data = []
...
class ILabProgress():
    ...
    def do_plot(self, ordered_vars, settings, setting_lists, variables, cfg):
        ...
        from ..store import parsers
        progress_key_properties = parsers.ILAB_PROGRESS_KEYS[cfg__progress_key]
