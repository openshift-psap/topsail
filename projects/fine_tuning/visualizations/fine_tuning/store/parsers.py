...
ILAB_SUMMARY_KEYS = {
    ...
    "average_throughput": types.SimpleNamespace(lower_better=False, units="samples/second", title="Execution wall-time"),
}
...
def _parse_ilab_logs(dirname):
    ...
    with (open(register_important_file(dirname, artifact_paths.FINE_TUNING_RUN_FINE_TUNING_DIR / "artifacts/pod.log")) as f):
        ...
        first_step_timestampt = datetime.datetime.fromisoformat(ilab_metrics.progress[0].timestamp)
        last_step_timestamp = datetime.datetime.fromisoformat(ilab_metrics.progress[-1].timestamp)
        period = (last_step_timestamp - first_step_timestampt).total_seconds()
