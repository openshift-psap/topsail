...
def prepare_common():
    ...
    if test_mode == "mcad":
        deploy_from = config.project.get_config("codeflare.mcad.deploy_from")
    ...
    if config.project.get_config("clusters.sutest.worker.fill_resources.enabled"):
...
def prepare_fill_workernodes():
    namespace = config.project.get_config("clusters.sutest.worker.fill_resources.namespace")
