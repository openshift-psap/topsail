...
# Forked and modified by Sagi Dayan <sdayan@redhat.com>
# Original file: https://github.com/ansible/ansible/blob/4ea8d9a7824827cf3d4a206599ffd7fe3a09eafd/lib/ansible/plugins/callback/junit.py (commit: 4ea8d9a)
...
class CallbackModule(CallbackBase):
    ...
    def _start_task(self, task):
        ...
        action = task.action
        recoverable = False
        rescue = task._parent.rescue if task._parent.rescue else []

        if len(task._parent.rescue):
            recoverable = task not in task._parent.rescue
        ...
        self._task_data[uuid] = TaskData(uuid, name, path, play, action, recoverable, rescue)
    ...
    def _build_test_case(self, task_data, host_data):
        ...
        name = 'Task:  %s/%s' % (task_data.play, task_data.name)
        ...
        if host_data.status == 'included':
            return TestCase(name, junit_classname, duration, str(host_data.result))
        ...
        if host_data.status == 'failed':
            if 'exception' in res and task_data.recoverable:
                message = "%s \n ==== ⚠️ Might be recovered by: %s" % (res['exception'].strip().split('\n')[-1], task_data.rescue)
                output = res['exception']
                test_case.add_error_info(message, output, 'Warning')
            elif 'msg' in res and task_data.recoverable:
                message = "%s \n ==== ⚠️ Might be recovered by: %s" % (res['msg'], task_data.rescue)
                test_case.add_error_info(message, dump, 'Warning')
            else:
                test_case.add_failure_info(message='rc=%s' % rc, output=dump)
        elif host_data.status == 'skipped':
            if 'skip_reason' in res:
                message = res['skip_reason']
            else:
                message = 'skipped'
            test_case.add_skipped_info(message, dump)
    ...
    def _generate_report(self):
        ...
        test_suite = TestSuite(name=self._playbook_name, test_cases=test_cases)
        report = to_xml_report_string([test_suite])

        output_file = os.path.join(self._output_dir, 'junit_%s.xml' % (self._playbook_name))
...
class TaskData:
    ...
    def __init__(self, uuid, name, path, play, action, recoverable=False, rescue=[]):
        self.uuid = uuid
        self.name = name
        self.path = path
        self.play = play
