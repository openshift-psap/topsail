...
generate_matbench::generate_visualization() {
    ...
    if ! matbench export_schema --file $ARTIFACT_DIR/notebooks.schema.json |& tee > "$ARTIFACT_DIR/_matbench_export_schema.log"; then
        _warning "An error happened while generating Notebook JSON Schema :/"
        retcode=1
    fi
